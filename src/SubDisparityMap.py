...
def generate_disparity_map(left_path, right_path, name, downsample_n=1, block_size=11, cmp_range=70):
    ...
    for i in range(offset, row_size - offset):
        ...
        for j in range(offset, col_size - offset):
            ...
            if (d > 0) & (d < max_col - 1):
                subl_prev = gray_left[i - offset:i + offset + 1, j - 1 + d - offset:j + d + offset]
                subl_next = gray_left[i - offset:i + offset + 1, j + 1 + d - offset:j + d + offset + 2]
                c1 = sum(sum(abs(subr - subl_prev)))
                c3 = sum(sum(abs(subr - subl_next)))

            # real_d = int((c3*(1-2*d) - c1*(1+2*d) + 4*d*c2)/(c1 + c3 - 2*c2))
            real_d = d - 0.5 * (c3 - c1) / (c1 + c3 - 2 * c2)

            disparity_matrix[i - offset, j - offset] = real_d
